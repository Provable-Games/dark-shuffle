use darkshuffle::models::battle::{Creature, Board, Battle, BattleEffects, BoardStats, RoundStats};
...
impl BoardUtilsImpl of BoardUtilsTrait {
    ...
    fn add_creature_to_board(creature: Creature, ref board: Board, ref board_stats: BoardStats) {
        ...
        if creature_type == CardType::Magical {
            board_stats.magical_count += 1;
        } else if creature_type == CardType::Brute {
            board_stats.brute_count += 1;
        } else if creature_type == CardType::Hunter {
    ...
    fn attack_monster(ref battle: Battle, ref board: Board, board_stats: BoardStats, ref round_stats: RoundStats) {
        if board.creature1.card_id != 0 {
            AttackUtilsImpl::creature_attack(ref board.creature1, ref battle, ref board, board_stats);
            round_stats.creature_attack_count += 1;
        }

        if board.creature2.card_id != 0 {
            AttackUtilsImpl::creature_attack(ref board.creature2, ref battle, ref board, board_stats);
            round_stats.creature_attack_count += 1;
        }

        if board.creature3.card_id != 0 {
            AttackUtilsImpl::creature_attack(ref board.creature3, ref battle, ref board, board_stats);
            round_stats.creature_attack_count += 1;
        }

        if board.creature4.card_id != 0 {
            AttackUtilsImpl::creature_attack(ref board.creature4, ref battle, ref board, board_stats);
            round_stats.creature_attack_count += 1;
        }

        if board.creature5.card_id != 0 {
            AttackUtilsImpl::creature_attack(ref board.creature5, ref battle, ref board, board_stats);
            round_stats.creature_attack_count += 1;
        }

        if board.creature6.card_id != 0 {
            AttackUtilsImpl::creature_attack(ref board.creature6, ref battle, ref board, board_stats);
            round_stats.creature_attack_count += 1;
        }
    }
    ...
    fn get_board_stats(board: Board, monster_id: u8) -> BoardStats {
        ...
        if board.creature1.card_id != 0 {
            let creature_type = CardUtilsImpl::get_card(board.creature1.card_id).card_type;
            if creature_type == CardType::Magical {
        ...
        if board.creature2.card_id != 0 {
            let creature_type = CardUtilsImpl::get_card(board.creature2.card_id).card_type;
            if creature_type == CardType::Magical {
                stats.magical_count += 1;
            } else if creature_type == CardType::Brute {
                stats.brute_count += 1;
        ...
        if board.creature3.card_id != 0 {
            let creature_type = CardUtilsImpl::get_card(board.creature3.card_id).card_type;
            if creature_type == CardType::Magical {
                stats.magical_count += 1;
            } else if creature_type == CardType::Brute {
                stats.brute_count += 1;
        ...
        if board.creature4.card_id != 0 {
            let creature_type = CardUtilsImpl::get_card(board.creature4.card_id).card_type;
            if creature_type == CardType::Magical {
                stats.magical_count += 1;
            } else if creature_type == CardType::Brute {
                stats.brute_count += 1;
        ...
        if board.creature5.card_id != 0 {
            let creature_type = CardUtilsImpl::get_card(board.creature5.card_id).card_type;
            if creature_type == CardType::Magical {
                stats.magical_count += 1;
            } else if creature_type == CardType::Brute {
                stats.brute_count += 1;
        ...
        if board.creature6.card_id != 0 {
            let creature_type = CardUtilsImpl::get_card(board.creature6.card_id).card_type;
            if creature_type == CardType::Magical {
                stats.magical_count += 1;
            } else if creature_type == CardType::Brute {
                stats.brute_count += 1;
    ...
    fn update_creatures(ref board: Board, creature_type: CardType, attack: u8, health: u8) {
        if board.creature1.card_id != 0 {
            if creature_type == CardType::All
                || creature_type == CardUtilsImpl::get_card(board.creature1.card_id).card_type {
        ...
        if board.creature2.card_id != 0 {
            if creature_type == CardType::All
                || creature_type == CardUtilsImpl::get_card(board.creature2.card_id).card_type {
        ...
        if board.creature3.card_id != 0 {
            if creature_type == CardType::All
                || creature_type == CardUtilsImpl::get_card(board.creature3.card_id).card_type {
        ...
        if board.creature4.card_id != 0 {
            if creature_type == CardType::All
                || creature_type == CardUtilsImpl::get_card(board.creature4.card_id).card_type {
        ...
        if board.creature5.card_id != 0 {
            if creature_type == CardType::All
                || creature_type == CardUtilsImpl::get_card(board.creature5.card_id).card_type {
        ...
        if board.creature6.card_id != 0 {
            if creature_type == CardType::All
                || creature_type == CardUtilsImpl::get_card(board.creature6.card_id).card_type {
    ...
    fn clean_board(ref battle: Battle, ref board: Board, board_stats: BoardStats) {
        if board.creature1.card_id != 0 && board.creature1.health == 0 {
            DeathUtilsImpl::creature_death(ref board.creature1, ref battle, ref board, board_stats);
            board.creature1 = Self::no_creature();
        }

        if board.creature2.card_id != 0 && board.creature2.health == 0 {
            DeathUtilsImpl::creature_death(ref board.creature2, ref battle, ref board, board_stats);
            board.creature2 = Self::no_creature();
        }

        if board.creature3.card_id != 0 && board.creature3.health == 0 {
            DeathUtilsImpl::creature_death(ref board.creature3, ref battle, ref board, board_stats);
            board.creature3 = Self::no_creature();
        }

        if board.creature4.card_id != 0 && board.creature4.health == 0 {
            DeathUtilsImpl::creature_death(ref board.creature4, ref battle, ref board, board_stats);
            board.creature4 = Self::no_creature();
        }

        if board.creature5.card_id != 0 && board.creature5.health == 0 {
            DeathUtilsImpl::creature_death(ref board.creature5, ref battle, ref board, board_stats);
            board.creature5 = Self::no_creature();
        }

        if board.creature6.card_id != 0 && board.creature6.health == 0 {
            DeathUtilsImpl::creature_death(ref board.creature6, ref battle, ref board, board_stats);
