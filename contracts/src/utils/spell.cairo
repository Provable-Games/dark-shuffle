...
impl SpellUtilsImpl of SpellUtilsTrait {
    fn cast_spell(
        card: Card, spell_details: SpellCard, ref battle: Battle, ref board: Array<Creature>, board_stats: BoardStats
    ) {
        ...
        if let CardDetails::spell_card(spell_card) = card.card_details {
            if CardUtilsImpl::_is_effect_applicable(spell_card.play_effect, card.card_type, board_stats) {
                CardUtilsImpl::apply_card_effect(
                    card.card_type, spell_card.play_effect, ref creature, ref battle, ref board, board_stats
                );
            }
        }
    }
